...
trap 'notify_fail "❌ <@367293674981294086> **evan-db-backup** failed on $(hostname) at $(date +%Y-%m-%dT%H:%M:%SZ). Check Nomad alloc logs."' ERR

DATE="$(date +'%Y-%m-%d_%H-%M-%SZ')"
BASENAME="evan-db-backup-$DATE.sql.gz"
PLAIN_PATH="$NOMAD_TASK_DIR/$BASENAME"
ENC_PATH="$PLAIN_PATH.gpg"

REMOTE="gdrive:backups/evan"
...
notify_ok "✅ **evan-db-backup** finished on \`$(hostname)\` at \`$(date +%Y-%m-%dT%H:%M:%SZ)\`. File: \`$BASENAME.gpg\` uploaded to \`$REMOTE\`."
SCRIPT
        ]
      ...
      template {
        ...
        env         = true
        data        = <<EOH
MYSQL_ROOT_PASSWORD={{ key "eqpaint/db/root/password" }}
MYSQL_DATABASE={{ key "eqpaint/db/name" }}
MYSQL_USER={{ key "eqpaint/db/user" }}
MYSQL_PASSWORD={{ key "eqpaint/db/password" }}
