...
trap 'notify "fail" "❌ <@367293674981294086> **linkwarden-backup** failed on $(hostname) at $(date +%Y-%m-%dT%H:%M:%SZ). Check Nomad alloc logs."' ERR
...
notify "ok" "✅ **linkwarden-backup** finished on \`$(hostname)\` at \`$(date +'%Y-%m-%d %H:%M:%SZ')\`. File: \`$BASENAME.gpg\` uploaded to \`$REMOTE\`."
SCRIPT
]
